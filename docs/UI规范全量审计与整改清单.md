...
- 验证：
  ...
  - 补充点击验收：从 `192.0.2.32` 切换到 `192.0.2.34` 后，新选中卡的 IP 主值和“可用”小字计算颜色均为主题 active text `rgb(66, 107, 87)`；原卡恢复普通主值色和 success text。验收后已恢复首选 `192.0.2.32`。
...
- 测试说明：
  1. 打开创建虚拟机弹框并展开候选 IP，记录前 3 张卡的顺序，例如 `192.0.2.32 / 192.0.2.34 / 192.0.2.35`。
  2. 点击第二张卡 `192.0.2.34`。
  3. 预期：顺序仍为 `.32 / .34 / .35`；只有第二张卡的选中外框、主题文字和“首选 192.0.2.34”提示变化，不应移动到第一张。
...
### 2026-07-29：设置页弱信息块归一

- 对应反馈：版本说明、Chrome 插件说明/代码、创建模板提示、当前方案和资源列表预览使用了相近的深色块背景，但边框、圆角、字体层级和主题联动口径不一致；期望统一采用更新页“版本说明”的弱块样式。
- 状态：已修复。
- 真实问题与根因：这些区域虽然大多读取 `--vrc-surface-muted`，但分别由 `.update-release-notes`、`.settings-empty-note`、`.appearance-theme-summary`、`.appearance-reading-sample` 和 `.chrome-extension-code` 独立维护；同类说明有的带边框、有的不带，短代码还固定消费终端绿黑背景，造成相同语义不同外观、不同主题下色相脱节。
- 实际改动：
  - `apps/web/src/styles.css` 新增共享 `.vrc-muted-block`：`10px 12px`、`7px` 圆角、无边框无阴影；背景、标题和正文分别消费 `--vrc-surface-muted`、`--vrc-text`、`--vrc-text-muted`。
  - `apps/web/src/App.vue` 将创建模板提示、Chrome 插件说明/短代码、维护空态、当前方案摘要和资源列表预览迁移到共享弱信息块；短代码只保留等宽字体，不再使用固定绿黑背景。
  - `apps/web/src/components/UpdateCenterPanel.vue` 让版本说明消费同一公共类，并保留组件自身的布局间距规则。
  - `docs/UI样式规范归档.md` 补充弱信息块的适用场景、禁用场景、主题 token 和真实 Console/状态块例外，后续不得按页面复制局部颜色。
- 测试说明：
  1. 打开“设置 / 更新”，确认版本说明为 muted 背景、7px 圆角、无外边框和阴影，标题为正文色、版本号与说明为 muted 字色。
  2. 打开“设置 / Chrome 插件”，确认 nginx 边界、存储口径和接口短代码与版本说明使用同款背景/圆角/无边框；代码仍为等宽字体，但不再出现固定绿黑大色块。
  3. 打开“设置 / 创建模板”和“设置 / 外观”，确认提示、当前方案、资源列表预览的外框均已去除，背景与版本说明同层；当前方案内部信息分隔线可以保留。
  4. 打开“设置 / 维护”，分别检查扫描中、无数据和错误文本；普通空态使用弱信息块，错误文字仍保持 danger 语义色。
  5. 依次切换石墨青、玄武铜、雾青、霓虹夜幕，并在浅色/深色间切换；所有弱信息块的背景、标题和说明文字必须同步变化，不出现固定绿黑、纯白或漂白边框。
  6. 打开真实 Console，确认终端画布仍使用独立 terminal 主题，没有被 `.vrc-muted-block` 改成设置页说明样式。
- 验证结果：
  - `npm --prefix apps/web run typecheck`、`npm --prefix apps/web run build` 和目标文件 `git diff --check` 均通过；构建仅保留现有 VueUse PURE 注释和 chunk 体积提示。
  - 浏览器实测石墨青浅色：当前方案与资源预览背景为 `rgb(238, 241, 232)`，正文为 `rgb(116, 123, 112)`，标题为 `rgb(39, 48, 42)`；边框 `0px`、圆角 `7px`、阴影 `none`、padding `10px 12px`。
  - 石墨青深色下背景切换为 `rgb(32, 42, 47)`，正文为 `rgb(147, 161, 170)`，标题为 `rgb(216, 224, 228)`；冰川光谱浅色下背景切换为 `rgb(237, 242, 247)`，确认背景与字体均由主题 token 联动。
  - Chrome 插件说明和短代码在深色实测中使用相同 muted 背景、无边框无阴影；短代码字体仍为 `SFMono-Regular / Consolas / monospace`。测试后已恢复原设置“石墨青 + 浅色”。

### 2026-07-29：Segmented 边框与选中投影归一

- 对应反馈：外观页“跟随系统 / 浅色 / 深色”和“默认 / 纯色 / 图片”切换控件是否应该显示边框不明确，截图中出现可见外框。
- 状态：已修复。
- 真实问题与根因：全局 segmented 已规定 `1px solid transparent` 作为盒模型占位，但外观页和创建预案局部样式将其覆盖为 `--vrc-border`；外观、VM 状态筛选、总览匹配、创建预案和定时任务又分别给选中滑块增加投影，导致同类切换控件有的有框、有的无框。
- 实际改动：
  - `apps/web/src/styles.css` 将外观页和创建预案 segmented 外层恢复为透明 1px 占位，清除 VM 状态筛选、总览匹配、外观切换、创建预案及锁定态选中块投影。
  - `apps/web/src/components/VmScheduleDialog.vue` 清除动作类型和执行周期 segmented 的局部选中投影。
  - `docs/UI样式规范归档.md` 明确：segmented 的 1px 是透明盒模型占位，不是可见边框；外层轨道和选中块均不得出现可见边框或阴影。
- 测试说明：
  1. 打开“设置 / 外观”，检查“界面明暗”和“背景类型”：外层只有 muted 背景，视觉上不显示边框；选中块只有 surface 背景与正文色，不显示边框或阴影。
  2. 切换各选项，确认外盒始终为 28px、选中块为 22px，上下各保留一致间距，切换时尺寸不跳动。
  3. 检查 VM 列表状态筛选、总览完全/模糊匹配、创建虚拟机预案切换、定时任务动作/周期：全部遵循同一规则。
  4. 切换石墨青、冰川光谱、霓虹夜幕及浅色/深色，确认 muted/surface/文字随主题变化，但透明边框不会显现成白边或深边。
- 验证结果：
  - `npm --prefix apps/web run typecheck`、`npm --prefix apps/web run build` 和目标文件 `git diff --check` 均通过；构建仅保留现有 VueUse PURE 注释和 chunk 体积提示。
  - 石墨青浅色实测：两组外层均为 `28px`、`padding: 2px`、`border-color: transparent`；选中背景为 `rgb(251, 251, 247)`，选中块 `border: 0px none`、`box-shadow: none`。
  - 石墨青深色实测：外层 muted 背景切换为 `rgb(32, 42, 47)`，选中背景切换为 `rgb(22, 29, 33)`，透明边框与无阴影规则保持不变；浏览器控制台无 error/warn。测试后已恢复“石墨青 + 浅色”。

### 2026-07-29：全系统切换控件统一收口

- 对应反馈：统一梳理系统里的切换开关，避免同类控件在不同页面出现边框、尺寸、关闭色和选中状态不一致。
- 状态：已修复。
- 审计范围与真实问题：
  - 正式代码共审计 17 个 `el-segmented` 入口和 11 个 `el-switch` 入口；原型目录不作为生产样式来源。
  - VM 状态、总览匹配、外观设置、创建预案和定时任务分别复制了 segmented 的颜色、尺寸、圆角与阴影契约；Chrome 服务位置、候选 IP 和创建预案锁定态又重新加了可见边框，后续容易继续漂移。
  - switch 全局只定义了 34×20px 普通规格，导致 4 个 `size="small"` 实际呈现为 34×20px 轨道配 12×12px 滑块；更新中心还单独使用更深的关闭色和无边框规则，定时任务重复声明尺寸。
- 本轮实际改动：
  - `apps/web/src/styles.css` 补齐 segmented 的 hover、pressed、disabled 和 selected-disabled 主题状态；外层保持透明 1px 占位，选中块无边框无阴影。
  - 清理 VM 状态、总览匹配、外观设置、Chrome 服务位置、候选 IP、创建预案和定时任务的页面级视觉特例；局部 class 仅保留宽度、flex 和响应式布局。
  - `apps/web/src/styles.css` 建立 switch 两档公共 token：普通 34×20px / 16px 滑块，密集 30×16px / 12px 滑块；两档统一固定宽度、主题填充轨道、透明占位边框、无常态阴影、disabled 透明度和键盘 focus outline。
  - `apps/web/src/components/UpdateCenterPanel.vue` 删除自动检查开关的局部关闭色、边框和尺寸覆盖；`apps/web/src/components/VmScheduleDialog.vue` 删除重复的 switch / segmented 视觉规则，并为两个任务开关补充可访问名称。
- 测试说明：
  1. 打开“设置 / 外观”，检查减少动效、光标闪烁、CLI/图形水印和拖拽节流：普通开关应为 34×20px、滑块 16px；底部三个摘要卡开关应为 30×16px、滑块 12px，均不得被文案挤压。
  2. 打开“设置 / 更新”，检查自动更新开关：关闭色应与外观页一致，不再单独变深；不支持自动更新时仍完整显示，并以 `opacity: 0.55` 表达 disabled。
  3. 打开 VM 定时任务 Dialog：状态一致时跳过使用普通规格，任务管理表的启用开关使用密集规格；动作、周期和 VM 状态 segmented 与其他页面保持 28/22px 契约。
  4. 打开 VM 列表、总览、Chrome 插件、创建虚拟机安装类型和候选 IP：所有 segmented 外层只显示 muted 填充，不显示可见外框；选中块无阴影，disabled 也不会重新出现边框。
  5. 分别用鼠标和 Tab 键测试 off/on、hover、pressed、focus 和 disabled：鼠标点击后无残留外框；Tab 聚焦时允许出现 2px 主题 outline，作为键盘定位反馈。
  6. 依次切换石墨青、玄武铜、冰川光谱和霓虹夜幕，并覆盖浅色/深色：switch 的 on/off、滑块，segmented 的背景与文字必须跟随主题 token，不得出现固定绿色、纯白或深灰特例。
- 验证结果：
  - 浏览器实测设置 / 外观浅色：普通 switch 计算尺寸为 `34×20px`、滑块 `16×16px`；摘要卡密集 switch 为 `30×16px`、滑块 `12×12px`。普通和密集轨道均为透明 1px 占位边框、`box-shadow: none`。
  - 浏览器实测设置 / 更新：自动检查 switch 为 `34×20px`，关闭/开启轨道与外观页消费同一主题规则，不再有更新中心专属关闭色或无边框覆盖。
  - 浏览器实测冰川光谱深色：segmented 外层为 `rgb(32, 42, 47)`，选中块为 `rgb(22, 29, 33)`；switch 关闭轨道为主题混合色；尺寸、透明边框和无阴影规则保持不变。测试后恢复石墨青浅色。
  - `npm --prefix apps/web run typecheck`、`npm --prefix apps/web run build`、目标文件 `git diff --check` 均通过；构建仅保留已有 VueUse PURE 注释和 chunk 体积提示。
  - 键盘 focus-visible 仍由 Element Plus 的 2px 主题 outline 规则提供；本轮未通过自动化强制触发焦点态，不把该状态冒充为已完成的交互截图验收。

### 2026-07-29：维护页扫描概览与介质列表层级优化

- 对应反馈：“扫描完成：记录 N 条，可清理 N 条”紧贴内容顶部，维护页信息层级扁平、细线和卡片较多，整体观感不协调。
- 状态：已修复。
- 真实问题与根因：
  - 扫描状态原本使用 `padding: 0 16px 10px`，顶部没有任何留白，直接贴在标题区底部分隔线上；状态、统计和列表之间仅依靠连续边线分隔，缺少明确分组。
  - 待处理介质的表头悬在数据卡片外，每条数据又使用独立四边框和圆角，形成“标题 + 表头 + 圆角卡”的多层碎片，与系统表格规范不一致。
- 本轮实际改动：
  - `apps/web/src/App.vue` 将扫描状态拆成“扫描状态”标签与结果正文，并为介质表头和数据行增加统一 `.maintenance-iso-table` 外层。
  - `apps/web/src/styles.css` 将扫描结果改为距标题 12px 的 34px 弱信息条；统计区增加底部留白，统计项最小高度调整为 54px。
  - 介质列表改为单一 1px 外框、7px 外圆角、32px muted 表头和连续 48px 数据行；数据行取消独立圆角及四边框，只保留表头/行之间的横向分隔线。
  - `docs/UI样式规范归档.md` 增加设置页异步状态条和结构化结果表格规则，后续同类页面复用该层级。
- 测试说明：
  1. 打开“设置 / 维护”，等待扫描完成；“扫描状态”信息条距标题区底线应为 12px，条内上下留白均匀，不得再贴顶或贴统计卡。
  2. 检查四项统计：保持四列等宽、54px 最小高度和完整 1px 弱边框；标签、数值不截断，也不与扫描条或列表标题相贴。
  3. 检查待处理介质：表头和数据行应位于同一个外框内；只有外层拥有 7px 圆角，单条记录不得再出现独立圆角卡片。
  4. 缩窄窗口到 980px 和 700px：统计区依次变为两列、单列；列表按既有规则隐藏平台/目标列，页面不得产生横向滚动。
  5. 切换石墨青浅色与霓虹夜幕深色：扫描条/表头使用主题 muted surface，统计项/表格行使用主题 surface，边框和正文随主题变化，不出现固定浅色背景。
- 验证结果：
  - 浏览器实测石墨青浅色：扫描条高 34px、上外边距 12px、`7px 10px` 内边距；统计区与列表之间层级清晰，表格外框为 1px、表头 32px、数据行 49px（含 1px 分隔线），页面横向溢出为 0。
  - 霓虹夜幕深色实测：扫描条/表头背景为 `rgb(36, 44, 55)`，统计项/表格/行背景为 `rgb(26, 32, 41)`，表格边框为 `rgb(52, 62, 76)`；测试后恢复石墨青浅色。
  - 本轮重新执行 `npm --prefix apps/web run typecheck`、`npm --prefix apps/web run build` 和目标文件 `git diff --check`，均通过；构建仅保留已有 VueUse PURE 注释与 chunk 体积提示。

### 2026-07-29：维护状态标点与连接列表信息降噪

- 对应反馈：维护状态条同时出现“扫描状态：扫描完成：”两个冒号；连接列表的“状态 / 操作”只重复表达当前选择，且缺少序号、表头与内容对齐不统一。
- 状态：已修复。
- 真实问题与根因：状态条标签和结果文案分别维护标点，组合后形成重复冒号；连接列表把“载入表单”包装成额外操作和伪运行状态，造成重复信息，并挤压名称与管理地址列。
- 本轮实际改动：
  - `apps/web/src/App.vue` 将维护状态条的冒号固定在“扫描状态：”标签，扫描完成和清理完成结果改用逗号连接统计内容，保证整句只有一个冒号。
  - 连接表删除“状态 / 操作”列，新增序号列，并改为 `序号 / 名称 / 平台 / 管理地址 / 端口 / 账号` 六列单行数据。所有表头与内容统一居中，点击整行继续载入连接信息。
  - 将“已保存连接 + 合计”与连接按钮合并为同一工具栏：标题和数量在左，动作在右；工具栏和表格之间统一 8px 节奏，不再拆出独立列表标题行。
  - `apps/web/src/styles.css` 删除已无调用的连接表文字操作按钮样式，只保留整行可点击光标；选中项继续使用共享主题 active text，不新增页面专属颜色或背景。
  - 连接页新增独立剩余高度列表区：页面和卡片固定，列表区以 `minmax(0, 1fr)` 占满剩余空间，只有 Element Table body 负责纵向滚动。
  - 当前行的六列数据统一切换为 `--vrc-active-text`；列表标题与“16 条”等合计信息统一在左侧以 8px 紧凑排列。
  - 连接行点击只切换选中项并回填上方表单，不再写入“选择连接”操作记录；真正的测试、保存、删除、加载资源仍按原业务动作记录日志。
  - `docs/UI样式规范归档.md` 补充连接库列结构、居中规则、选中表达和异步状态条单冒号规则。
- 测试说明：
  1. 打开“设置 / 维护”，等待扫描完成，状态条应显示为“扫描状态：扫描完成，记录 N 条，可清理 N 条”，整句只出现一个冒号；执行清理后同样不得出现双冒号。
  2. 打开“设置 / 连接”，确认表格为 `序号 / 名称 / 平台 / 管理地址 / 端口 / 账号` 六列，不再显示“状态 / 操作”及“选择 / 已选 / 当前使用 / 可选择”。
  3. 检查六列全部采用单行展示，表头和内容均居中；名称、地址和账号长内容按表格 tooltip 规则处理，管理地址下方不得再拼接端口或账号副行。
  4. 依次点击不同连接行，确认上方连接表单随之切换，当前行六列数据统一使用主题 active text；不得出现选中背景、加粗或“已选”文案，点击也不触发加载资源、保存、测试、删除或新增操作记录等额外动作。
  5. 切换石墨青浅色、霓虹夜幕深色和一个非绿色主题，确认选中名称、表头、表格边框和状态条文字随主题 token 变化，无硬编码颜色。
  6. 准备超过可视高度的连接数据，滚动列表时设置标题、连接表单、动作按钮和“已保存连接”标题必须固定；只有表格数据行滚动，页面右侧不得出现第二条纵向滚动条。
  7. 检查“已保存连接 + 数量”和按钮处于同一工具栏，标题/合计在左、动作在右；工具栏上下及其到表格的间距约 8px，不再出现按钮单独一行或列表标题另起一行。
- 验证结果：
  - `npm --prefix apps/web run typecheck`、`npm --prefix apps/web run build` 和目标文件 `git diff --check` 均通过；构建仅保留已有 VueUse PURE 注释与 chunk 体积提示。
  - 浏览器在 1280×720、16 条连接下实测：连接页和卡片不产生外层滚动；表格 body 为 307px / 768px、`overflow-y: auto`，只有数据行滚动。
  - 六列计算宽度分别为 58px、197px、104px、219px、76px、127px，合计适配 781px 表格内容区；表格 body 的 `clientWidth / scrollWidth` 均为 781px，不再产生横向滚动。
  - 首行实测为“1 / xenserver-3 / XenServer / 192.168.2.6 / 22 / root”，数据行高 48px；六项均为独立单行单元格，DOM 中不存在“状态 / 操作”列，也不存在端口、账号拼入管理地址的副行。
  - 连续切换 `xenserver-3` 与 `xenserver-5` 后，上方表单和当前行同步变化，操作记录数量保持不变；确认行选择只承担本地选中和表单回填。
  - 石墨青浅色下当前行六列数据统一消费 `--vrc-active-text`，无选中背景、加粗或额外状态文案；主题切换验证沿用共享 active token，不设置连接页专属颜色。
  - 工具栏实测高 39px，上下外边距均为 8px；“已保存连接”和“16 条”间距为 8px，按钮组高 30px，按钮组到表格间距为 8px。测试结束后停留在“设置 / 连接”。

### 2026-07-29：IP 池列表外框去除试验（已撤回）

- 对应反馈：IP 池左侧表格外框与两栏区域边界叠加，先去掉列表自身外框查看减轻线框后的实际效果。
- 状态：已撤回。实测后外框消失导致 muted 表头像悬空色块，搜索框、表头和数据行缺少完整收口，整体更松散。
- 试验内容：曾仅对 `.ip-pool-list` 清除表格自身四边边框与圆角，保留 muted 表头、数据行横向分割线、左侧搜索框和列表/编辑区之间的竖向分割线；用户确认效果不佳后已恢复原外框与圆角，其它设置表格始终未受影响。
- 测试说明：
  1. 打开“设置 / IP 池”，确认左侧表格不再显示包围表头和数据行的四边外框，表头与三条数据仍能靠横向分割线分组。
  2. 检查搜索框、左侧栏边缘和右侧编辑区：搜索框边框保留，两栏之间仍只有一条贯穿竖线，其它页面的连接表、日志表边框不应变化。
  3. 切换浅色和深色主题，确认表头背景、行分割线和选中文字继续使用主题 token，不出现固定白边或深边。

### 2026-07-29：IP 池主从导航结构归一（已按 Git 撤回）

- 对应反馈：IP 池左侧把名称、地址范围和网关塞入单列表格的双行复合单元格，窄栏内容被截断；仅去掉表格外框后表头又形成悬空色块，整体更松散。
- 状态：已撤回。用户确认单行导航方案不符合预期，已按当前分支 `HEAD` 中的 IP 池模板和样式逐块恢复。
- 真实问题与根因：左侧区域的业务职责只是选择一个 IP 池，完整字段已经在右侧编辑区展示；将其套成单列表格并重复摘要，增加了无意义表头、复合行和内部外框，且 268px 栏宽无法容纳完整范围和网关。
- 实际改动：
  - 左侧改为原生按钮组成的单行导航列表，删除 `el-table`、`IP 池` 表头及地址范围/网关副行；每项只显示名称。
  - 导航行统一为 40px，仅保留顶部和行间弱分割线；选中项只使用 `--vrc-active-text`，无选中背景、加粗、卡片边框或状态文案。hover 使用弱背景，键盘 focus 仅使用名称细下划线，不叠加矩形外框。
  - 保留搜索、左右贯穿分割线及右侧完整编辑表单；导入、导出、新增、复制、删除、清空和保存逻辑均未改动。
  - `docs/UI样式规范归档.md` 增加主从编辑页面的单行导航例外，避免把只承担对象选择的列表机械套用结构化数据表格规范。
- 测试说明：
  1. 打开“设置 / IP 池”，确认左侧搜索框下方直接显示三个 40px 单行名称，不再出现“IP 池”表头、双行地址范围、网关摘要或列表卡片外框。
  2. 依次点击三个名称，右侧标题和表单字段必须同步切换；当前名称只变为主题色，其背景、字重、行高和位置不得变化。
  3. 搜索名称、IP 段或网关，确认筛选能力保持不变；无结果时显示统一弱文字空态。
  4. 使用 Tab 和 Enter/Space 操作列表，确认每项可聚焦、可选择；键盘焦点使用名称细下划线，不出现矩形外框或阴影。
  5. 切换浅色、深色及非绿色主题，确认普通文字、选中文字、hover 背景、focus 下划线和分割线全部跟随主题 token。
- 验证结果：
  - 浏览器在 1280×720 下实测 3 个导航项均为 `247×40px`，DOM 中表头和 table 数量均为 0；列表只保留 1px 顶线和行间分割线。
  - 当前项背景为透明、字重为 400，底部分割线与普通项同为 `rgb(217, 222, 208)`；当前文字使用石墨青主题 active text，焦点无矩形 outline 和 box-shadow。
  - 点击“192.168.2 通用网段”后右侧标题和字段同步切换；搜索 `127` 后导航项收敛为 1 条，清空搜索后恢复 3 条，最终恢复选中“192.168.129 隔离网段”。
  - 自动化浏览器未可靠模拟 Tab 焦点顺序，未将键盘 Enter/Space 冒充为已验收；原生 button 语义及 `aria-current` 已通过 DOM 核对。
  - 撤回结果：恢复 Git 基线的双行按钮卡片、名称/地址范围/网关结构、50px 最小行高、7px 间距和 1px/7px 单项外框；同时恢复 IP 池页面原始内容区与标题 class。连接、主题、日志等同文件其它未提交改动未被覆盖。

### 2026-07-29：Chrome 本地连接库列结构归一

- 对应反馈：Chrome 插件“本地连接库”列表信息层级松散，名称单元格复行展示账号和最近使用时间，固定的“存储”口径重复占列，操作按钮常态危险色过强。
- 状态：已修复。
- 真实问题与根因：本地连接表仍使用早期复合单元格结构，和设置 / 连接已经归一的单行数据列不一致；`local encrypted` 对所有记录均相同，不具备逐行比较价值；删除操作在未交互时已经呈现红色，破坏普通信息与危险动作的主次关系。
- 本轮实际改动：
  - `apps/web/src/App.vue` 将表格调整为 `序号 / 名称 / 平台 / 管理地址 / 账号 / 最近使用 / 状态 / 操作` 八列，全部单行并居中；删除“存储”列，将 `Host` 中文化为“管理地址”。
  - 空态文案统一为“暂无本地连接”，继续使用 Element Table empty 区消费剩余高度，不新增卡片、按钮或额外说明。
  - `apps/web/src/styles.css` 删除名称复行单元格样式，清除 Element Table 内层底边伪元素，列表只保留所在面板的一套完整外框；删除按钮常态使用 `--vrc-text-muted`，hover / focus 才切换 `--vrc-danger`。
  - Chrome 设置卡增加内容容器查询：可用内容宽度不超过 900px 时，列表与新增表单改为上下排列，不再只看浏览器 viewport 导致 1280px 窗口中的表格实际仅剩约 409px；上下布局的空态最小高度收紧为 140px，并排布局仍铺满剩余高度。
  - `docs/UI样式规范归档.md` 增加 Chrome 本地加密连接库的固定列、存储口径、空态和危险操作规则。
- 测试说明：
  1. 打开“设置 / Chrome 插件 / 本地连接库”，宽屏并排布局的空数据状态确认表头为八列，空态显示“暂无本地连接”，空态区连续铺到左侧面板底部，不出现第二张卡片或底部空白带；上下布局时空态应保持紧凑。
  2. 新增一条仅用于本机验证的连接，确认一行依次显示序号、名称、平台、管理地址、账号、最近使用、状态和删除图标；名称下方不得再出现账号或时间副行，表格中不得出现“存储 / local encrypted”列。
  3. 检查表头和内容均居中，数据行保持 48px；长名称、管理地址、账号和最近使用时间按单行省略与 tooltip 展示，不撑高行高。
  4. 检查删除图标：常态为统一弱操作色，hover 或键盘 focus 时才变为危险色；删除流程仍使用原确认逻辑。
  5. 将内容卡缩窄到可用宽度不超过 900px（1280×720 默认壳布局即可复现），确认左右布局改为上下排列，宽表只在表格内部横向滚动，设置页本身不产生横向滚动。
  6. 切换浅色、深色和非绿色主题，确认表头、正文、空态、状态文字、操作图标和边框均由主题 token 驱动。
- 验证结果：
  - `npm --prefix apps/web run typecheck`、`npm --prefix apps/web run build`、`npm --prefix apps/web test` 和目标文件 `git diff --check` 均通过；Web 测试 29/29，构建仅保留既有 VueUse PURE 注释与 chunk 体积提示。
  - Impeccable 设计规则扫描 `apps/web/src/App.vue` 返回 0 条问题。
  - 1280×720 石墨青浅色实测：Chrome 设置卡可用宽度 798px，容器查询将左右布局改为 772px 单列；表格宽度与内部内容宽度均为 781px，页面横向溢出为 0。空态高度为 140px，下方新增表单完整进入同一内容滚动区。
  - 临时新增“本地测试连接”后，DOM 与截图均确认八列单行展示；全部表头居中，数据行高 48px，名称下方不存在账号/时间副行，也不存在“存储”列。删除图标常态计算色为 `rgb(116, 123, 112)`，消费主题弱文字色。
  - 霓虹夜幕实测：表格背景 `rgb(26, 32, 41)`、表头背景 `rgb(36, 44, 55)`、空态文字 `rgb(163, 175, 191)`、面板边框 `rgb(52, 62, 76)`，均随主题 token 切换。
  - 测试连接已通过原“确认删除”流程清理；主题已恢复为“石墨青 + 浅色”。未执行任何 XenServer、虚拟机、存储或网络变更。

### 2026-07-29：页面定位更正，IP 池调整恢复

- 对应反馈：上一条记录将截图中的连接页误记为 IP 池页。
- 状态：已核实无需改（IP 池本轮误改已恢复）。
- 实际处理：`apps/web/src/styles.css` 已恢复 IP 池左栏 `10px` 内边距、`border-right`、列表 `7px` 间距、编辑标题 `44px / 9px 11px`、表单 `10px 12px 12px`、操作区 `12px` 及窄屏底部边框规则；未改 IP 池业务结构和数据逻辑。
- 说明：此前为 IP 池编写的“透明间距替代内部竖线”规范已删除，避免将已撤回方案作为通用规则。

### 2026-07-29：连接设置表单与列表分割线、间距收口

- 对应反馈：连接设置表单与“已保存连接”之间的横向分割线存在感过强，表单字段呈 2/2/2 布局，视觉节奏割裂。
- 状态：已修复。
- 真实问题与根因：`.connection-settings-actions` 继承 `.settings-actions` 的 `border-top`，在表单和列表之间形成额外横线；表单使用三列并让管理地址、密码跨列，最终呈现为三行两列组合。
- 本轮实际改动：
  - `apps/web/src/styles.css` 为连接动作区显式移除 `border-top`，将间距收口为上下 `8px / 10px`，保留左侧合计、右侧动作的单行工具栏。
  - 宽屏连接表单改为六列单行：连接名、平台、管理地址、端口、账号、密码；管理地址和密码使用更宽轨道，端口保持窄列。使用连接卡容器查询，不足 `1100px` 自动改为三列两行，不足 `640px` 改为两列，不足 `460px` 改为单列，避免内容挤压。
  - `docs/UI样式规范归档.md` 补充连接表单的字段顺序、轨道宽度、容器响应式断点及表单与工具栏无分割线规则。
- 测试说明：
  1. 打开“设置 / 连接”，确认表单与“已保存连接”之间不再有横向分割线；动作按钮与合计信息保持同一行，表单底部留白约 8px。
  2. 在宽屏检查六个字段是否同处一行，字段顺序保持“连接名 / 平台 / 管理地址 / 端口 / 账号 / 密码”，管理地址和密码不得被压缩到不可读。
  3. 缩窄到 980px 以下，确认表单平稳降为两列；缩窄到 700px 以下时变为单列，页面不出现横向滚动。
  4. 切换浅色和深色主题，确认字段边框、文字、动作区间距和列表表头继续使用主题 token；不出现固定白线或阴影。
- 验证结果：
  - 浏览器实测 1280×720：连接卡可用宽度为 `809px`，表单自动降为三列两行，六个字段宽度均约 `256px`，密码字段完整可见；`连接名 / 平台 / 管理地址` 位于第一行，`端口 / 账号 / 密码` 位于第二行，不再形成 2/2/2 组合。
  - 动作区 `border-top-width` 为 `0px`、`box-shadow` 为 `none`；“已保存连接 + 数量”与操作按钮保持同一行，表单到底部工具栏之间只保留紧凑留白。
  - 霓虹夜幕深色实测：输入背景为 `rgb(26, 32, 41)`、表格表头为 `rgb(36, 44, 55)`、表格边框为 `rgb(52, 62, 76)`；动作区仍为无边线、无阴影。测试后已恢复石墨青浅色。
  - `npm --prefix apps/web run typecheck`、`npm --prefix apps/web test`（29/29）、`npm --prefix apps/web run build`、`git diff --check` 均通过；Impeccable 扫描 `apps/web/src/App.vue` 返回 `[]`。构建仅保留既有 VueUse PURE 注释和 chunk 体积提示。
  - 未执行 XenServer、虚拟机、存储或网络变更。

### 2026-07-29：左侧连接空状态文本块归一

- 对应反馈：左侧连接列表为空时，“还没有连接”文本块仍显示旧卡片边框，未跟随已统一的弱文本块样式。
- 状态：已修复。
- 真实问题与根因：空状态仅使用 `.sidebar-empty`，该页面类仍独立维护背景、边框、圆角和 padding，没有消费共享 `.vrc-muted-block`。
- 本轮实际改动：
  - `apps/web/src/App.vue` 为连接空状态增加共享 `.vrc-muted-block`。
  - `apps/web/src/styles.css` 删除 `.sidebar-empty` 重复的背景、边框、圆角和 padding，只保留侧栏布局、间距和外边距；同时从旧 surface 分组中移除该选择器。
  - 现在文本块统一使用主题 muted surface、`10px 12px` 内边距、7px 圆角、无边框、无阴影、12px 常规标题与 11px 辅助文字。
- 测试说明：
  1. 在没有保存连接的状态下查看左侧连接区，“还没有连接”块应保留弱背景和圆角，但不得出现四周描边或阴影。
  2. 标题应为 12px 常规字重，说明为 11px muted 文字；块区左右外边距保持 14px，不贴搜索框或侧栏边缘。
  3. 切换浅色与深色主题，背景和文字必须随 `--vrc-surface-muted / --vrc-text / --vrc-text-muted` 变化，不出现固定灰色或白边。
- 验证结果：
  - 模板命中已核对：空状态同时消费 `.sidebar-empty` 和 `.vrc-muted-block`；页面类只保留 `display / gap / margin`，共享类统一提供背景、padding、圆角、字体和无边框/无阴影规则。
  - 当前测试环境存在 16 条已保存连接，未通过删除或篡改数据强制触发空态，避免改变用户连接数据；本项未把合成截图冒充为真实空态截图。
  - `npm --prefix apps/web run typecheck`、`npm --prefix apps/web test`（29/29）、`npm --prefix apps/web run build` 和目标文件 `git diff --check` 均通过；Impeccable 扫描返回 `[]`。构建仅保留既有 VueUse PURE 注释和 chunk 体积提示。
  - 未执行 XenServer、虚拟机、存储、网络或连接库数据变更。

### 2026-07-29：小尺寸响应式重构方案（待实施）

- 对应反馈：窗口缩小后，左侧连接区占据首屏，设置导航和表单被纵向拉长；物理机总览的指标、查询工具栏与表格之间出现大段空白，页面同时出现多条纵向滚动条。
- 状态：部分完成。2026-07-29 已先实施不改业务结构的最小优化；Drawer、紧凑功能栏和移动顶栏等完整重构仍待后续评估，执行 agent 不得把整节写成“已修复”。
- 方案原则：这不是把桌面布局继续缩窄或增加 `flex-wrap`，而是按可用内容宽度切换应用外壳。核心功能在小屏仍保留，但连接清单、设置分组等次级导航改为按需展开。

#### 本轮已完成的最小优化

- `<=700px` 的完整纵向连接树改为单行横向连接条，保留资源总览和全部连接切换能力；侧栏实测由接近半屏/整屏收敛到约 `214px`。
- 小屏 App Shell 使用 `100dvh` 的两行 Grid，主工作区成为唯一纵向内容滚动区；页面整体和工作区横向溢出均为 `0`。
- `<=980px` 的 8 项设置导航改为 `44px` 高的单行横滑导航，并补齐 `auto + minmax(0, 1fr)` 行轨道，消除 `701-980px` 下约 `180px` 的导航空白。
- 总览在手机宽度固定为两列指标，`<=380px` 降为一列；移动工具栏清除桌面 `flex-basis` 在纵轴产生的约 `178px` 空白。
- 本轮没有新增 Drawer、移动顶栏、Vue 状态或连接加载逻辑；这些较大改造继续按后文章节执行，不应混入本次最小修复。

#### A. 已核实的根因

1. `apps/web/src/styles.css` 的 `@media (max-width: 700px)` 将 `.app-shell` 改成 `display: block`，同时让 `.sidebar` 保留最高 `48vh`、连接列表保留最高 `28vh`。结果是完整连接清单固定堆在工作区上方，设置或总览每次都要先越过半屏侧栏。
2. 小屏下 `.workspace`、`.settings-workspace-panel`、`.settings-workspace-layout` 和 `.settings-workspace-content` 全部改成 `height: auto / overflow: visible`，而连接列表本身仍可滚动；浏览器页面、侧栏列表、Element Table 因此形成多个同向滚动容器。
3. 设置页有 8 个分组。`<=980px` 仍使用 `repeat(8, minmax(92px, 1fr))` 的完整横向导航，只增加横向滚动；条目长短差异和不可见溢出使导航像被压扁的多列工具栏。
4. `.overview-panel` 和 `.single-host-view` 仍使用 `height: 100vh`，总览表格又设置 `height="100%"`；它们在已经堆叠了侧栏的移动文档流中继续占一个完整视口，造成大空白和滚动高度失真。
5. 总览与 VM 指标只改变列数，没有同步调整页面头、工具栏、空态高度和表格滚动边界；移动端最终仍是“桌面卡片 + 桌面表格”的长页面。

#### B. 统一响应模式

| 可用宽度 | App Shell | 连接入口 | 设置分组 | 主内容 |
|---|---|---|---|---|
| `>=1180px` | 保留 `220px + minmax(0, 1fr)` 双栏 | 完整连接树常驻 | `210px` 左侧纵向导航 | 桌面密度、内容区单纵滚动 |
| `768-1179px` | 左侧改为约 `64px` 功能栏，工作区占剩余宽度 | 功能栏显示总览、当前连接、连接列表、设置、日志图标；连接列表由 Drawer 展开 | 改为单行横向滚动 tabs | 保留表格，表格内部横滚；页面只保留一个纵滚动容器 |
| `<768px` | 不再渲染占文档流高度的完整侧栏，改为 `52-56px` sticky 顶栏 | 顶栏显示品牌、当前连接和菜单按钮；连接列表由左侧全高 Drawer 打开 | 页面标题下使用 sticky 横向滚动 tabs | 单列内容；正文纵滚，表格只横滚 |

- 断点以内容开始失真为准，不通过 User-Agent 区分设备；`768px` 左右仍要覆盖平板横竖屏和桌面高缩放场景。
- Drawer 打开/关闭不得重置 `workspaceMode`、当前连接、设置分组、搜索条件或主内容滚动位置。
- 触摸设备的菜单、返回、设置、连接项和 tabs 点击区域不得小于 `44px`；密集表格行内按钮保留现行规格，但不能成为页面主导航入口。

#### C. App Shell 正确改法

1. 在 `apps/web/src/App.vue` 增加响应式导航状态，例如 `connectionDrawerVisible`；将侧栏内部的“品牌头、搜索、总览入口、连接分组、操作记录”拆成可被桌面侧栏和 Drawer 复用的一个组件或同一模板片段，不能维护两份连接业务逻辑。
2. `>=1180px` 使用现有 `.sidebar`。`768-1179px` 只保留窄功能栏，不在栏内展示全部连接名称。`<768px` 隐藏侧栏占位，新增 `.mobile-app-bar`；菜单按钮使用现有图标组件并补 `aria-label="打开连接列表"`。
3. Drawer 宽度建议 `min(340px, calc(100vw - 24px))`；内容顺序保持“搜索 -> 资源总览 -> 平台分组/连接 -> 操作记录”，关闭后焦点返回触发按钮。
4. 根布局始终保持一个可计算视口高度：优先使用 `100dvh` 并以 `100vh` 回退；不要在手机断点把 `.app-shell` 改成无限高的普通文档流。
5. 纵向滚动归属固定为：Drawer 内连接列表独立滚动；主工作区独立滚动；表格只负责其数据区或横向滚动。任何时刻同一可见区域不能同时出现 body、workspace、panel 三条纵向滚动条。

建议结构（名称可按项目现状调整）：

```vue
<main class="app-shell">
  <DesktopSidebar v-if="isWideShell" />
  <CompactRail v-else-if="isTabletShell" @open-connections="connectionDrawerVisible = true" />
  <MobileAppBar v-else @open-connections="connectionDrawerVisible = true" />
  <section class="workspace">...</section>
  <el-drawer v-model="connectionDrawerVisible" class="connection-drawer">...</el-drawer>
</main>
```

实现时不要直接依赖 `window.innerWidth` 散落判断。使用单一 composable / `matchMedia` 状态控制需要改变 DOM 或焦点语义的部分；纯视觉重排仍由 CSS media/container query 完成。

#### D. 设置页正确改法

1. `apps/web/src/App.vue` 将 8 个设置分组收敛为配置数组，桌面纵向导航与小屏 tabs 消费同一数据源，避免标签、更新红点、图标和 active 判断重复。
2. `>=1180px` 保留左侧设置导航；`<1180px` 改为 `.settings-workspace-tabs`：`display:flex; overflow-x:auto; flex-wrap:nowrap; scrollbar-gutter:auto`，每项 `flex:0 0 auto`，不得继续使用 8 列等分 grid，也不得换成两行/三行按钮墙。
3. 激活分组时调用 `scrollIntoView({ block: "nearest", inline: "nearest" })`，保证“维护/日志”等尾部项目可见；tabs 本身加 `aria-label="设置分组"`，按钮保留 `aria-current="page"` 或等价 tabs 语义。
4. 手机端设置头压缩为一行：返回按钮使用图标按钮，标题保留；长说明移到当前设置内容头，不与返回按钮争宽。tabs 可 sticky 在移动顶栏下方。
5. `<768px` 移除设置页外层 desktop panel 的四边框和圆角，只保留各真实内容区的必要分隔。表单一列化，但动作区不能跟着字段无限下沉：保存/测试等当前分组主动作放在内容底部 sticky action bar，使用 `env(safe-area-inset-bottom)` 处理底部安全区。
6. 连接表和 Chrome 本地连接表仍保留表格语义；小屏只允许表格容器横向滚动，不把列强行压成不可读宽度，也不要擅自隐藏账号、状态或操作等核心字段。

#### E. 总览与 VM 列表正确改法

1. 删除移动文档流中的 `.overview-panel { height: 100vh; min-height: 620px; }` 和 `.single-host-view { height/min-height: 100vh; }` 影响。桌面模式可通过网格剩余轨道计算高度；移动模式使用 `height:auto; min-height:0`。
2. 总览指标：桌面 6 列，平板 3 列，手机稳定 2 列；只有 `<=360px` 才降为 1 列。不要使用会在相邻宽度间频繁跳列的无限 `auto-fit`。卡片值必须单行，说明允许一行省略，卡片高度保持稳定。
3. 工具栏顺序固定为“列表标题/合计 -> 搜索 -> 匹配模式 -> 动作”。手机端标题与合计一行，搜索独占一行，匹配 segmented 与三个图标动作处于下一行或同一紧凑命令栏；不能用 `justify-content: space-between` 在纵向布局中拉开大段空白。
4. Element Table 保留原始列宽并由 `.overview-table-scroll` / `.vm-table-wrap` 横向滚动；外层页面必须 `overflow-x:hidden`。不要把结构化资源表强制改成卡片列表，也不要通过隐藏滚动条掩盖列宽问题。
5. 空态使用 `min-height: clamp(180px, 32dvh, 360px)`；加载、无连接、无搜索结果分别保留现有语义，但不得占固定 `520/620px`。
6. `<768px` 下表格头部和主页面头可以 sticky，但只保留一个 sticky 层级；测试 iOS Safari 地址栏收缩后不能遮挡第一行数据。

#### F. CSS 组织要求

1. 不在 `styles.css` 末尾继续堆一组覆盖旧规则。执行 agent 先合并当前 `@media (max-width: 980px)`、`@media (max-width: 700px)` 中与 Shell、设置、总览有关的规则，再建立明确的 desktop/tablet/mobile 三段。
2. 建议新增布局 token：`--vrc-app-bar-height`、`--vrc-rail-width`、`--vrc-settings-tabs-height`，安全区直接使用 `env(safe-area-inset-*)`；颜色、控件高度和圆角继续消费现有 token。
3. 对设置卡内部两栏、表单列数、Chrome 连接库、IP 池等“内容宽度决定是否变形”的组件优先使用 container query；App Shell 与移动顶栏使用 viewport media query。
4. `apps/web/index.html` 的 viewport 增加 `viewport-fit=cover`，并在 sticky 顶栏/底部动作区处理安全区；不能把安全区 padding 全局加到 body 后再让桌面端继承。

#### G. 分派与验收

- 建议由 **Agent A：Shell / 总览** 负责 App Shell、连接 Drawer、总览/VM 工具栏和表格滚动；由 **Agent C：Theme / Feedback / Console** 负责设置页导航、单滚动容器和安全区 token。两个 agent 不得同时修改同一响应式媒体块；主 agent 先拆分 CSS 区域或约定合并顺序。
- 本项不涉及 API、连接数据格式、XenServer 查询或任何资源变更；不得以响应式整改为由修改后端和连接加载业务。
- 必测 viewport：`1440x900`、`1180x820`、`1024x768`、`768x1024`、`430x932`、`390x844`、`360x800`、`320x568`，以及 `844x390` 手机横屏。
- 每个尺寸同时验收：页面无整体横向滚动；正文只有一个主纵滚动条；Drawer 可用鼠标、触摸、Esc 和键盘关闭；设置尾部 tabs 可达；长连接名/IPv6/中文文案不撑破；虚拟键盘弹出后当前输入与 sticky action bar 不互相遮挡。
- 浏览器至少覆盖 Chrome、Safari；Electron 窗口还要验证 macOS 标题栏拖拽区，移动顶栏和 Drawer 的按钮必须标记 `-webkit-app-region: no-drag`。
- 最低自动化：为 Shell 三种模式、设置 tabs 可见性与 Drawer 状态保持补组件/E2E 测试；使用 Playwright 对上述关键尺寸截图并断言 `document.documentElement.scrollWidth === document.documentElement.clientWidth`。真实手机触摸与虚拟键盘仍需人工验收，不能用桌面缩放截图代替。

### 2026-07-29：日志表格结构与执行详情归一

- 对应反馈：日志缺少序号，时间和事件未居中；详情只能看到对象摘要，无法判断操作经过哪个接口、平台实际执行了什么动作。
- 状态：已修复。
- 真实问题与根因：设置页使用三列表格，完整记录 Dialog 仍使用独立卡片列表；`ActivityEntry` 仅保存标题、对象和说明，API 入口与 provider 执行摘要均未进入返回契约。
- 本轮实际改动：
  - 新增 `apps/web/src/components/ActivityLogTable.vue`，设置页和完整记录 Dialog 统一复用 `序号 / 时间 / 事件 / 详情 / 状态` 五列表格；序号、时间、事件和状态居中，详情使用摘要加查看图标。
  - 日志详情弹窗展示序号、时间、状态、事件、对象、说明、执行入口和可选平台命令；日志搜索同时覆盖执行入口和平台命令。
  - `ActivityEntry` 增加 `request / command`，读取总览、物理机、VM、镜像、连接测试、维护清理、创建预检、改名、扩容和 VM 动作等记录写入脱敏 API 入口。
  - `VmActionResult` 增加可选 `command`：XenServer 返回实际尝试的安全 `xe` 摘要，PVE 返回 API 方法与路径，VMware 返回 SOAP 动作；不返回密码、令牌、完整脚本或用户控制台输入。
  - 删除完整记录 Dialog 原有逐条圆角卡片样式，状态仅通过主题语义文字色表达；详情命令块复用主题 muted surface，无额外边框和阴影。
- 测试说明：
  1. 打开“设置 / 日志”，确认表头依次为“序号、时间、事件、详情、状态”，序号从当前结果第一行开始递增，时间和事件的表头与内容均居中。
  2. 点击任意行详情列右侧查看图标，确认弹窗显示序号、时间、状态、事件、对象和说明；读取总览记录应显示 `POST /api/inventory/hosts`。
  3. 点击“查看完整记录”，确认搜索、状态筛选和同一五列表格共存，不再出现逐条圆角日志卡片；搜索 API 路径或平台动作也能命中。
  4. 在获得变更授权后执行一条测试 VM 电源操作，再打开成功记录：XenServer 应显示脱敏 `xe` 命令，PVE 显示 `/api2/json/...` 请求，VMware 显示 SOAP 动作。任何平台都不得出现密码、令牌、Cookie 或完整含密脚本。
  5. 切换浅色和深色主题，表头、边框、详情图标、状态文字、详情命令块均随主题 token 变化；详情弹窗不得产生横向滚动。
- 验证结果：
  - 浏览器在 1280×720 石墨青浅色下实测：设置页表头为“序号 / 时间 / 事件 / 详情 / 状态”，3 条记录序号为 1-3；时间、事件和状态表头及内容均居中，详情查看按钮为 26px 透明图标按钮。
  - “总览加载完成”详情实测包含序号、时间、状态、事件、对象、说明和执行入口，入口显示 `POST /api/inventory/hosts`；完整记录 Dialog 同样复用五列表格，搜索和六项状态筛选保留。
  - 霓虹夜幕实测：表格背景 `rgb(26, 32, 41)`、表头 `rgb(36, 44, 55)`、边框 `rgb(52, 62, 76)`、详情图标 `rgb(163, 175, 191)`；测试后恢复石墨青浅色。
  - 未为视觉验收执行任何 VM 开机、关机、重启、删除、改名或扩容；平台命令通过 provider 返回契约、前后端类型检查和构建验证，不把未授权变更冒充为运行验收。
  - `npm --prefix apps/web run typecheck`、`npm --prefix apps/api run typecheck`、Web 测试 29/29、API 测试 135/135、前后端生产构建及目标文件 `git diff --check` 均通过；Impeccable 扫描返回 `[]`。Web 构建仅保留既有 VueUse PURE 注释和 chunk 体积提示。

### 2026-07-29：更新页版本说明块基线恢复

- 对应反馈：更新页优化后，版本说明块相对运行平台行和底部“上次检查”左右缩进，三个内容区不再对齐。
- 状态：已修复。
- 真实问题与根因：版本说明复用 `.vrc-muted-block` 后，设置页单卡片的全局规则将其外边距强制覆盖为 `14px 16px 16px`，覆盖了更新组件原有的 `10px 0 0`，造成左右各多出 16px 缩进。
- 本轮实际改动：
  - `apps/web/src/styles.css` 将单卡片弱文本块外边距改为读取 `--vrc-muted-block-margin`，未定义时仍保持原 `14px 16px 16px`，其他设置块布局不变。
  - `apps/web/src/components/UpdateCenterPanel.vue` 在更新卡片上声明 `--vrc-muted-block-margin: 10px 0 0`；版本说明继续复用统一弱块背景、字体和主题 token，只由父卡片决定布局间距，不复制共享视觉样式。
- 测试说明：
  1. 打开“设置 / 更新”，在存在版本说明时确认运行平台行、版本说明块和底部“上次检查”左右边缘位于同一基线，版本说明不得再额外缩进 16px。
  2. 检查版本说明与运行平台行的垂直间距为 10px，块内仍保持 `10px 12px`；背景无边框、无阴影，圆角、标题和辅助文字继续符合弱文本块规范。
  3. 切换浅色与深色主题，确认版本说明背景和字体随 `--vrc-surface-muted / --vrc-text / --vrc-text-muted` 变化，布局基线不因主题改变。
  4. 切换到“创建模板”等同样含直接弱文本块的设置页，确认未声明自定义间距的块仍保持原 `14px 16px 16px` 外边距，避免公共规则产生回归。
- 验证结果：
  - 浏览器 Web 运行态实测：运行平台行与“上次检查”的左右坐标均为 `469px / 1252px`，宽度均为 `783px`；更新卡片计算得到 `--vrc-muted-block-margin: 10px 0 0`。Web 服务没有桌面客户端版本说明数据，因此该块未渲染，本轮未通过伪造桌面更新状态冒充运行截图；版本说明的最终 margin 由已核对的同一父卡片变量和全局消费规则共同确定。
  - `npm --prefix apps/web test` 通过，共 29/29；`npm --prefix apps/web run build` 通过，已包含 `vue-tsc -b` 类型检查，构建仅保留既有 VueUse PURE 注释和 chunk 体积提示。
  - 两个本轮组件的 Impeccable 扫描返回 `[]`，目标文件 `git diff --check` 通过；全量扫描历史 `styles.css` 仍有 1 个既有 Arial 字体提示和 6 个既有布局属性动画提示，均不在本轮改动位置，未混入本项整改。

### 2026-07-29：创建虚拟机 IP 池下拉宽度自适应

- 对应反馈：创建虚拟机展开“网络配置 / IP 池”后，下拉选项浮层明显窄于触发框，网段名称与地址范围被截断，未按内容自适应。
- 状态：已修复。
- 真实问题与根因：`fit-input-width` 已将下拉内部内容宽度计算为触发框宽度，但 teleport 到 body 的最外层 `.el-select__popper` 仍保留 Element Plus 默认窄宽度；外层同时设置 `overflow: hidden`，导致更宽的内部内容被裁切。IP 池下拉还遗漏了创建向导统一的 0 间距和隐藏浮层箭头配置。
- 本轮实际改动：
  - `apps/web/src/styles.css` 将创建向导共享下拉浮层外层统一改为 `width: auto !important`，由内部触发框/内容测量结果决定实际宽度，并将最大宽度限制为 `min(920px, 100vw - 48px)`；浮层继续使用主题弱边框且无阴影。
  - `apps/web/src/components/VmProvisioningDialog.vue` 为 IP 池下拉补充专属 `vrc-provision-ip-pool-dropdown` 标识，保留 `fit-input-width`，并统一设置 `offset=0`、隐藏箭头；镜像与 IP 池下拉现在遵循同一浮层契约。
- 测试说明：
  1. 打开任意物理机的“虚拟机信息”，点击“创建虚拟机”，展开“网络配置 / IP 池”并打开下拉框；浮层左边缘应与触发框对齐，宽度至少覆盖触发框，不再出现外层窄于选项内容的裁切。
  2. 检查“192.168.2 通用网段”等选项：左侧名称和右侧起止地址应完整显示；内容超过可用视口时才允许单行省略，浮层不得越出 Dialog/视口安全边界。
  3. 检查浮层与触发框垂直间距为 0，不显示箭头和阴影，只保留 1px 主题弱边框；下拉向上翻转时也保持同样间距。
  4. 同时打开系统镜像下拉，确认长镜像名称仍可按内容扩展、短内容不产生固定大面积空白，避免共享规则只修复 IP 池却回归镜像下拉。
  5. 切换浅色和深色主题，确认下拉背景、边框、主标题、地址范围和选中态均随主题 token 变化。
- 验证结果：
  - 浏览器在 1280px 视口实测：IP 池触发框宽 `422.27px`、浮层宽 `422px`，左坐标同为 `34.5px`；内部下拉内容宽 `420px`，三个可见选项的 `scrollWidth` 与 `clientWidth` 均为 `412px`，名称和地址范围没有被裁切。
  - 浮层向上翻转时与触发框间距约 `0.09px`（亚像素取整），箭头数量为 0，计算阴影为 `none`；截图确认浮层右边缘与触发框对齐，候选 IP 区未被遮挡。
  - `npm --prefix apps/web test` 通过，共 29/29；`npm --prefix apps/web run build` 通过，已包含 `vue-tsc -b` 类型检查，构建仅保留既有 VueUse PURE 注释和 chunk 体积提示。
  - 两个本轮组件的 Impeccable 扫描返回 `[]`，目标文件 `git diff --check` 通过；本项没有调用 PING、创建虚拟机或任何 XenServer / VMware / PVE 变更操作。
