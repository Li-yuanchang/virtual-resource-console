...
| API / BFF | `apps/api` | Fastify + TypeScript 本地服务，负责 Provider 编排、连接存储、资源清单、Provisioning、安装源发布、控制台网关、IP 租约和 IP 池策略 |
...
| IP 池与识别策略 | `ipPoolPolicy.ts`, `runtimePolicy.ts`, `config/ip-pools.example.json` | 默认 DNS、地址池、物理机网段匹配、可分配主机号和 VM 名称 IP 推断 |
...
当使用 `YOUR_UPDATE_HOST`、`192.0.2.31` 这类内网 VM 作为 Chrome 插件测试服务器时，不需要额外启动 Vite，也不需要 nginx。当前 `YOUR_UPDATE_HOST` 实际部署方式是：离线自包含包 + 内置 Node runtime，直接由 VRC API 在 `3988` 端口托管 `/api/*`、Web dist、Chrome CRX 和 `update.xml`。
...
```text
YOUR_UPDATE_HOST:22    可连接
YOUR_UPDATE_HOST:3987  端口开放，但 /api/health 返回空响应，不像 VRC API
192.0.2.31:22    可连接
192.0.2.31:3987  端口开放，但 /api/health 返回空响应，不像 VRC API
YOUR_UPDATE_HOST nginx  systemctl is-active nginx 返回 unknown；无 nginx 进程
YOUR_UPDATE_HOST:3988  runtime/node/bin/node apps/api/dist/index.js 正在监听
```

因此测试部署不建议直接覆盖现有 `3987` 进程。当前 `YOUR_UPDATE_HOST` 先用 `3988` 跑 VRC API，并由 API 同端口托管 Web、Chrome 插件 CRX 和 `update.xml`。

当前 `YOUR_UPDATE_HOST` 实测部署状态：

```text
部署方式：离线自包含包，不依赖 Docker / yum / 服务器 npm install
运行地址：http://YOUR_UPDATE_HOST:3988/
健康检查：http://YOUR_UPDATE_HOST:3988/api/health
应用目录：/opt/vrc/app
...
Chrome update URL：http://YOUR_UPDATE_HOST:3988/chrome-extension/update.xml
Chrome CRX URL：http://YOUR_UPDATE_HOST:3988/chrome-extension/virtual-resource-console-extension-0.1.1.crx
Chrome 开发者模式 zip：http://YOUR_UPDATE_HOST:3988/chrome-extension/virtual-resource-console-extension-0.1.3.zip
Windows 下载目录：http://YOUR_UPDATE_HOST:3988/downloads/windows/
```
...
4. 打开插件设置页，服务地址填 `http://YOUR_UPDATE_HOST:3988` 或 `http://192.0.2.31:3988`。
...
当前 `YOUR_UPDATE_HOST` 已托管开发者模式 zip 下载：

```text
http://YOUR_UPDATE_HOST:3988/chrome-extension/virtual-resource-console-extension-0.1.3.zip
```
...
- 不能直接依赖浏览器 `crypto.subtle`。`http://YOUR_UPDATE_HOST:3988` 这类非 localhost 的 HTTP 页面不是安全上下文，Chrome 不暴露 WebCrypto subtle；Web UI 必须保留纯 JS 加密兜底。
...
2026-07-17 对 `http://YOUR_UPDATE_HOST:3988/` 的实测结论：
...
| PVE 129.2 | 本机到 `198.51.100.2:8006` 可达，但 YOUR_UPDATE_HOST API 代连超时；按服务器到 129 网段路由问题排查 |
...
Update URL：http://YOUR_UPDATE_HOST:3988/chrome-extension/update.xml
CRX URL：http://YOUR_UPDATE_HOST:3988/chrome-extension/virtual-resource-console-extension-0.1.1.crx
```
...
<gupdate xmlns="http://www.google.com/update2/response" protocol="2.0">
  <app appid="hiljahiejjbfbhalfoohiajfikhngfml">
    <updatecheck codebase="http://YOUR_UPDATE_HOST:3988/chrome-extension/virtual-resource-console-extension-0.1.1.crx" version="0.1.1" />
  </app>
</gupdate>
```
...
"1"="hiljahiejjbfbhalfoohiajfikhngfml;http://YOUR_UPDATE_HOST:3988/chrome-extension/update.xml"
```
...
```json
{
  "scripts": {
    ...
    "predist:mac": "node ../../scripts/prepare-electron-runtime.mjs darwin-arm64 && node ../../scripts/prepare-electron-api-runtime.mjs",
    "dist:mac": "electron-builder --mac",
    "predist:win": "node ../../scripts/prepare-electron-runtime.mjs win-x64 && node ../../scripts/prepare-electron-api-runtime.mjs",
    "dist:win": "electron-builder --win --x64"
  },
  "build": {
    "appId": "com.virtualresource.console",
    "productName": "VRC",
    "afterPack": "../../scripts/ensure-macos-app-signature.cjs",
    "releaseInfo": {
...
#### 4.4.1 YOUR_UPDATE_HOST 内网更新打包与发布

截至 2026-07-28，`0.1.52` 已按以下链路完成 macOS ARM64 与 Windows x64 内网更新验证。现场 YOUR_UPDATE_HOST 实际访问地址为 `http://YOUR_UPDATE_HOST:3988/desktop-updates`。

YOUR_UPDATE_HOST 的 `vrc-api.service` 当前未显式配置 `VRC_DESKTOP_UPDATE_DIR`，服务端代码会按 `VRC_DATA_DIR` 默认值读取 `/root/.virtual-resource-console/desktop-updates/`。除非同步修改 systemd 环境变量，否则发布包必须推到这个目录；不要再按旧文档推到 `/srv/virtual-resource-console/desktop-updates/`。

当前桌面更新是完整 Electron 包更新，不是无需重启的 Web 热更新。客户端只异步读取更新清单；`autoDownload=false`，发现版本后由用户手动下载，下载完成后重启替换应用。

Windows `0.1.43` 发布物曾出现“文件名和清单为 `0.1.43`、`app.asar/package.json` 仍为 `0.1.39`”的问题，该版本的 Windows ZIP 和 Setup 均不得继续分发。修复版本从 `0.1.44` 开始，发布脚本会读取 `win-unpacked/resources/app.asar` 并校验内嵌版本；不一致时直接终止发布。

Windows 分发必须区分两类模式：

1. `VRC Setup <version>.exe` 是安装版。首次手工安装允许选择目录，安装目录存在 `Uninstall VRC.exe`，客户端可以使用 NSIS 自动更新；更新过程复用原安装目录，不再次显示目录选择。
2. `VRC-<version>-win.zip` 和 `VRC <version>.exe` 是免安装版。免安装版不执行 NSIS 自动更新，用户需要退出全部 VRC 进程后下载新版 ZIP 并替换目录。

`dist:win` 的前置脚本必须清理旧的 `win-unpacked`、Windows Setup、portable、ZIP 和 `latest.yml`，防止 electron-builder 从旧输出目录重新封装。发布验收不能只检查文件名和清单版本，必须同时检查 ZIP、Setup 与 `win-unpacked` 内部的 `app.asar/package.json`。

打包前必须同时调整：

1. `apps/electron/package.json` 的版本号。
2. `package-lock.json` 中 `apps/electron` 的版本号。
3. `apps/electron/release-notes.md` 的用户可见版本说明。

禁止覆盖同版本更新包。已经发布或安装过的版本必须递增版本号，否则客户端不会重新识别更新，缓存和 blockmap 也可能与清单不一致。

构建命令：

```bash
npm --workspace apps/web run typecheck
npm --prefix apps/electron install --no-audit --no-fund
...
test -f .electron-dist/electron-v38.2.2-win32-x64.zip || \
  curl -L --fail --retry 3 \
    -o .electron-dist/electron-v38.2.2-win32-x64.zip \
    https://npmmirror.com/mirrors/electron/38.2.2/electron-v38.2.2-win32-x64.zip
mkdir -p .electron-dist/win32-x64
ditto -x -k .electron-dist/electron-v38.2.2-win32-x64.zip .electron-dist/win32-x64
CSC_IDENTITY_AUTO_DISCOVERY=false \
ELECTRON_BUILDER_BINARIES_MIRROR=https://npmmirror.com/mirrors/electron-builder-binaries/ \
../../node_modules/.bin/electron-builder --win --x64 --publish never \
  -c.electronDist=./.electron-dist/win32-x64

cd ../..
node scripts/prepare-online-update-release.mjs
```

本机打包时禁止从仓库根目录直接执行当前根 `package.json` 的 `dist:mac` / `dist:win`；根 `package.json` 可能不是 monorepo 入口，容易让 hook 路径解析错误。Electron 打包必须从 `apps/electron` 子包执行，并显式指定 `electronDist`，避免 electron-builder 在默认 Electron / NSIS / DMG 依赖下载链路中卡住。

macOS 与 Windows 必须分别在构建前执行对应的 `predist:*` 脚本，不能复用另一个平台构建后残留的 `.runtime/`。macOS 包内必须是 `resources/runtime/node-darwin-arm64`，Windows 包内必须是 `resources/runtime/node-win-x64`；如果 macOS zip/DMG 体积异常变小，优先解包检查 runtime 平台是否错误。已经上传过的错误版本不得覆盖，必须递增版本重新发布。

Windows 跨平台打包的 Electron runtime 压缩包必须缓存在 `apps/electron/.electron-dist/electron-v38.2.2-win32-x64.zip`。后续打包只允许清理并重新解压 `.electron-dist/win32-x64/`，不要删除缓存 zip；只有缓存 zip 缺失或 Electron 版本升级时才重新下载。

关键产物为：

```text
apps/electron/dist/VRC-<version>-arm64-mac.zip
apps/electron/dist/VRC-<version>-arm64-mac.zip.blockmap
apps/electron/dist/VRC-<version>-arm64.dmg
apps/electron/dist/VRC-<version>-arm64.dmg.blockmap
apps/electron/dist/latest-mac.yml
```

没有 Apple Developer ID 的内网包通过 `scripts/ensure-macos-app-signature.cjs` 在 `afterPack` 阶段补固定 ad-hoc designated requirement：

```text
designated => identifier "com.virtualresource.console"
```

该策略只用于内网测试和受控分发，目的是让 Sparkle/Squirrel.Mac 更新前后能够比较稳定的应用标识；正式外部分发仍必须使用 Developer ID 签名和 notarization。

发布前验证：

```bash
VERSION=0.1.45
grep -n -A 12 'version:\|releaseNotes' apps/electron/dist/latest-mac.yml
npx asar list apps/electron/dist/mac-arm64/VRC.app/Contents/Resources/app.asar \
  | grep '^/release-notes.md$'
codesign --verify --deep --strict apps/electron/dist/mac-arm64/VRC.app
codesign -dr - apps/electron/dist/mac-arm64/VRC.app 2>&1

tmpdir=$(mktemp -d /tmp/vrc-update.XXXXXX)
ditto -x -k "apps/electron/dist/VRC-${VERSION}-arm64-mac.zip" "$tmpdir"
codesign --verify --deep --strict "$tmpdir/VRC.app"
codesign -dr - "$tmpdir/VRC.app" 2>&1
```

发布顺序必须是“包和 blockmap 在前，清单在后”。`latest-mac.yml` 是客户端看到新版本的入口，如果先上传清单，客户端可能在大文件尚未传完时开始下载不完整包。

```bash
VERSION=0.1.52
UPDATE_DIR=/root/.virtual-resource-console/desktop-updates

tar -C "release/online-update-${VERSION}/desktop-updates" \
  --exclude='latest-mac.yml' \
  --exclude='latest.yml' \
  -cf - . \
  | ssh root@YOUR_UPDATE_HOST "mkdir -p ${UPDATE_DIR} && tar -C ${UPDATE_DIR} -xf -"

scp \
  "release/online-update-${VERSION}/desktop-updates/latest-mac.yml" \
  "release/online-update-${VERSION}/desktop-updates/latest.yml" \
  root@YOUR_UPDATE_HOST:${UPDATE_DIR}/
```

GitHub Release 可以作为版本归档和有仓库权限人员的手动下载入口，发布资产可包含 macOS / Windows 安装包、blockmap、`latest-mac.yml`、`latest.yml` 和服务器离线包。但当前客户端热更新源仍以 YOUR_UPDATE_HOST 内网 HTTP 更新源为准，不从 GitHub Release 自动更新。

如果仓库是私有仓库，GitHub Release 资产只适合已登录且有权限的人员手动下载。不要为了私有 Release 自动更新把 GitHub token 内置到客户端包里；客户端 token 可被用户从安装包、运行时或请求中提取，可能暴露私有仓库代码和资产权限。若未来需要公网下载但代码保持私有，应使用单独的公开下载仓库、对象存储/CDN 或继续使用受控内网更新源。

发布后必须比较本地与远端 SHA-256，并从 HTTP 更新入口读取最终清单：

```bash
VERSION=0.1.52
shasum -a 256 "release/online-update-${VERSION}/desktop-updates/VRC-${VERSION}-arm64-mac.zip"
ssh root@YOUR_UPDATE_HOST \
  "sha256sum /root/.virtual-resource-console/desktop-updates/VRC-${VERSION}-arm64-mac.zip"
curl -fsS http://YOUR_UPDATE_HOST:3988/desktop-updates/latest-mac.yml
curl -fsS http://YOUR_UPDATE_HOST:3988/desktop-updates/latest.yml
curl -fsSI "http://YOUR_UPDATE_HOST:3988/desktop-updates/VRC-${VERSION}-arm64-mac.zip"
curl -fsSI "http://YOUR_UPDATE_HOST:3988/desktop-updates/VRC%20Setup%20${VERSION}.exe"
```

验收口径：

1. HTTP 清单版本、文件名、大小和 `releaseNotes` 与本地构建一致。
2. 本地与 YOUR_UPDATE_HOST 的 ZIP SHA-256 完全一致。
3. 构建目录 `.app` 和 ZIP 解包后的 `.app` 都通过 `codesign --verify --deep --strict`。
4. 两者 designated requirement 都是 `com.virtualresource.console`。
5. 旧客户端能检测新版本、完成下载、进入“待重启”，重启后版本号变化。
6. 更新完成后仍可从包内 `release-notes.md` 查看当前版本说明。
...
5. 未配置 Authenticode 证书时，Setup 和主程序均为未签名 PE，只能用于允许手工放行的受控内网测试；如果 SmartScreen、AppLocker 或企业终端策略禁止运行，必须配置受信任的代码签名证书，不能通过修改 NSIS 页面规避。
