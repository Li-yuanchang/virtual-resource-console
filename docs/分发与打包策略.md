...
因此测试部署不建议直接覆盖现有 `3987` 进程。当前 `208` 先用 `3988` 跑 VRC API，并由 API 同端口托管 Web、Chrome 插件 CRX 和 `update.xml`。
...
| PVE 129.2 | 本机到 `198.51.100.2:8006` 可达，但 208 API 代连超时；按服务器到 129 网段路由问题排查 |
...
截至 2026-07-28，`0.1.52` 已按以下链路完成 macOS ARM64 与 Windows x64 内网更新验证。现场 208 实际访问地址为 `http://YOUR_UPDATE_HOST:3988/desktop-updates`。

208 的 `vrc-api.service` 当前未显式配置 `VRC_DESKTOP_UPDATE_DIR`，服务端代码会按 `VRC_DATA_DIR` 默认值读取 `/root/.virtual-resource-console/desktop-updates/`。除非同步修改 systemd 环境变量，否则发布包必须推到这个目录；不要再按旧文档推到 `/srv/virtual-resource-console/desktop-updates/`。
...
GitHub Release 可以作为版本归档和有仓库权限人员的手动下载入口，发布资产可包含 macOS / Windows 安装包、blockmap、`latest-mac.yml`、`latest.yml` 和服务器离线包。但当前客户端热更新源仍以 208 内网 HTTP 更新源为准，不从 GitHub Release 自动更新。
