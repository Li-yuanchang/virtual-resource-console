...
IP pools 是强制配置：运行时只读取 `ip-pools.json`，不读取旧 `providerIpPools`，文件缺失或格式不正确必须报错。桌面客户端通过打包资源 `config/ip-pools.example.json` 在首次启动时写入数据目录；服务器部署需要在上线步骤里执行 `cp config/ip-pools.example.json "${VRC_DATA_DIR:-$HOME/.virtual-resource-console}/ip-pools.json"`，之后由现场自行维护。
