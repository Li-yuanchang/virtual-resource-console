...
| Chrome 插件 | 作为轻入口，连接本机或内网 VRC API；不承载 Node 后端、SSH、XenAPI 或长任务 |
...
| IP 池配置 | `ipPoolPolicy.ts`, `config/ip-pools.json` | 默认 DNS、地址池、物理机网段匹配规则和可分配主机号 |
...
日志文件：/opt/vrc/app/logs/vrc-api.log 或历史部署脚本约定的 logs/vrc.out.log、logs/vrc.err.log
...
cd /opt/vrc/app
mkdir -p logs
nohup env HOST=0.0.0.0 PORT=3988 VRC_RUNTIME_MODE=web \
  runtime/node/bin/node apps/api/dist/index.js >> logs/vrc-api.log 2>&1 &
```
...
6. 在“本地连接库”录入测试服务器连接，连接信息只进入当前浏览器 `chrome.storage.local`；打开控制台时只向 VRC API 创建内存态 `ephemeral connection`。
...
由于该 VM 是 CentOS 7、glibc 2.17，且无 Docker / Podman / Node / npm、yum 无外网 DNS，当前测试部署随包内置 `node-v20.19.5-linux-x64-glibc-217` 运行时。后续如果改成 Docker 或 systemd，需要先补服务器基础环境。

切换到正式端口时，先确认现有 `3987` 进程可停止，再改为：

```bash
cd /opt/vrc/app
mkdir -p logs
nohup env HOST=0.0.0.0 PORT=3987 VRC_RUNTIME_MODE=web \
  runtime/node/bin/node apps/api/dist/index.js >> logs/vrc-api.log 2>&1 &
...
当前最小插件只保存 API 地址、展示偏好和临时 UI 状态。后续如果要求“平台连接信息只留在浏览器本地”，需要启用私密连接模式：插件本地加密保存连接信息，VRC API 只接收临时连接会话，不把平台账号、密码写入共享服务器磁盘。
...
3. 生产环境优先使用 HTTPS 远端 API；本机模式允许 `127.0.0.1`。
...
该模式只属于 Chrome 插件配置项，不替代 Web、macOS、Windows 的运行方式。Web 页面继续作为 VRC API 的主界面；macOS / Windows 继续由 Electron 客户端内置 API、Web dist 和 Node runtime；Chrome 插件只保存浏览器侧入口地址、本地密文连接和临时会话状态。
...
```text
POST /api/ephemeral-connections
  输入：插件解密后的连接信息
  输出：临时 connectionId
  存储：API 内存，默认 30 分钟过期，不写 connectionStore

GET /api/inventory/*?connectionId=<ephemeral-id>
POST /api/console/* 使用临时 connectionId
```
...
4. VRC API 只在一次操作或短 TTL 内使用凭据，不能写入 `~/.virtual-resource-console/connection-store`。
...
IP pools 是强制配置：运行时只读取 `ip-pools.json`，不读取旧 `providerIpPools`，文件缺失或格式不正确必须报错。桌面客户端通过打包资源 `config/ip-pools.json` 在首次启动时写入用户目录；服务器部署需要在上线步骤里执行 `cp config/ip-pools.json ~/.virtual-resource-console/ip-pools.json`，之后由现场自行维护。
...
2. Chrome 插件和 Web UI 默认连接远端 HTTPS API。
3. 连接凭据、runtime policy、任务状态的持久化和备份策略。
