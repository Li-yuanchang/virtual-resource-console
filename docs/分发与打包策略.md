...
- 不能直接依赖浏览器 `crypto.subtle`。`http://192.0.2.26:3988` 这类非 localhost 的 HTTP 页面不是安全上下文，Chrome 不暴露 WebCrypto subtle；Web UI 必须保留纯 JS 加密兜底。
...
2026-07-17 对 `http://192.0.2.26:3988/` 的实测结论：
...
| PVE 129.2 | 本机到 `198.51.100.2:8006` 可达，但 2.26 API 代连超时；按服务器到 129 网段路由问题排查 |
...
#### 4.4.1 2.26 内网更新打包与发布

截至 2026-07-23，`0.1.45` 已按以下链路完成 macOS ARM64 内网更新验证。2.26 更新源为 `http://192.0.2.26:3988/desktop-updates`，服务器目录为 `/srv/virtual-resource-console/desktop-updates/`。
...
`dist:mac` 会先重新构建 API 和 Web，再准备 macOS Node runtime、API 生产依赖并调用 electron-builder。关键产物为：
...
```bash
VERSION=0.1.45
scp -i ~/.ssh/vrc_update_key \
  "apps/electron/dist/VRC-${VERSION}-arm64-mac.zip" \
  "apps/electron/dist/VRC-${VERSION}-arm64-mac.zip.blockmap" \
  "apps/electron/dist/VRC-${VERSION}-arm64.dmg" \
  "apps/electron/dist/VRC-${VERSION}-arm64.dmg.blockmap" \
  deploy@example-update-host:/srv/virtual-resource-console/desktop-updates/

scp -i ~/.ssh/vrc_update_key \
  apps/electron/dist/latest-mac.yml \
  deploy@example-update-host:/srv/virtual-resource-console/desktop-updates/latest-mac.yml
```
...
```bash
VERSION=0.1.45
shasum -a 256 "apps/electron/dist/VRC-${VERSION}-arm64-mac.zip"
ssh -i ~/.ssh/vrc_update_key deploy@example-update-host \
  "sha256sum /srv/virtual-resource-console/desktop-updates/VRC-${VERSION}-arm64-mac.zip"
curl -fsS http://192.0.2.26:3988/desktop-updates/latest-mac.yml
curl -fsSI "http://192.0.2.26:3988/desktop-updates/VRC-${VERSION}-arm64-mac.zip"
```

验收口径：

1. HTTP 清单版本、文件名、大小和 `releaseNotes` 与本地构建一致。
2. 本地与 2.26 的 ZIP SHA-256 完全一致。
