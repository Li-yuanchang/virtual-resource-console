import type { IsoImage } from "./types.js";
...
function isCentos7HostDvdLabel(value: string): boolean {
  ...
  // XenServer only exposes the mounted DVD volume label for host media, so the exact ISO
  // filename is unavailable even when the inserted installer is the maintained CentOS 7 disc.
