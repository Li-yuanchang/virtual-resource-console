import type { IpPoolConfig, ProviderType } from "../types";

/** Validates the network settings required before probing or provisioning addresses. */
export function validateProvisioningIpPool(pool: IpPoolConfig, providerType: ProviderType): string {
  ...
  if (providerType !== "xenserver") return "";
  if (!pool.gateway) return settingsMessage("XenServer 无人值守安装必须配置网关");
  if (!isIpv4Cidr(pool.cidr)) return settingsMessage("XenServer 无人值守安装必须配置有效 CIDR");
  ...
  if (!pool.dns.some(isIpv4)) return settingsMessage("XenServer 无人值守安装必须配置有效 DNS");
  return "";
}
